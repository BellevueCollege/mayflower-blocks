					...
					>
						...
						<SelectControl
							...
							onChange={ ( gridColumnSize ) => {
								...
								const size = parseInt( gridColumnSize, 10 );
				...
				attributes: {
					...
					gridColumnText: {
						...
						shortcode: ( attrs, { content } ) => {
							...
							const filtered = content.replace( /(\[\s*column\s*.*?\]\s*)|(\s*\[\s*\/\s*column\s*\])/gmi, '' );
					...
					gridColumnClass: {
						type: 'string',
						shortcode: ( attrs, { content } ) => {
							const columnRx = /(\w{2})(?=\s*=)/gmi;
							const filtered = content.match( columnRx );
