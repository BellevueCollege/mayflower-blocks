...
registerBlockType( 'mayflower-blocks/row', {
	...
	edit: function( { className, attributes, setAttributes, isSelected } ) {
		...
		const currentBlockClientId = select( 'core/block-editor' ).getSelectedBlockClientId(); // returns this block's client id
		const currentBlockData = select( 'core/block-editor' ).getBlock( currentBlockClientId ); // returns this block's data; includes children and attributes
		...
		const handleAddColumnBlock = () => {
			...
			if ( Array.isArray( currentBlockChildren ) ) {
				if ( attributes.childColumns >= 0 && attributes.childColumns < 6 ) {
					...
					const gridColumnSize = ( 12 % ( attributes.childColumns + 1 ) == 0 ) ? ( 12 / ( attributes.childColumns + 1 ) ) : 4;
					...
					dispatch( 'core/block-editor' ).insertBlock( columnBlock, currentBlockData.innerBlocks.length, currentBlockClientId );
					...
					currentBlockChildren.forEach( child => {
						...
						dispatch( 'core/block-editor' ).updateBlockAttributes( child.clientId, { siblingColumns: attributes.childColumns + 1 } );

						if ( 12 % ( attributes.childColumns + 1 ) == 0 ) { // sets size for 1, 2, 3, 4, 6 columns
							dispatch( 'core/block-editor' ).updateBlockAttributes( child.clientId, { gridColumnSize: gridColumnSize } );
						} else { // sets size to adjust for 5 columns
							dispatch( 'core/block-editor' ).updateBlockAttributes( child.clientId, { gridColumnSize: 2 } );
						}
					} );
				}
			}
			...
			wp.data.dispatch( 'core/block-editor' ).selectBlock( currentBlockClientId );
		};
			...
			if ( Array.isArray( currentBlockChildren ) ) {
				...
				currentBlockChildren.forEach( child => {
					if ( child.attributes.isEditing == false ) {
						dispatch( 'core/block-editor' ).updateBlockAttributes( child.clientId, { isVisible: true } );
					} else {
						dispatch( 'core/block-editor' ).updateBlockAttributes( child.clientId, { isEditing: false } );
		...
		const handleAddRowBlock = () => {
			...
			const rowIndex = select( 'core/block-editor' ).getBlockIndex( currentBlockClientId );
			// Insert New Block to the current block appending to the the previous row block
			dispatch( 'core/block-editor' ).insertBlock( rowBlock, rowIndex + 1 );
		};
		 ...
		 */
		const RowMenu = (
			<div className="row-menu" style={ { marginTop: attributes.childColumns !== 0 ? '1em' : '' } }>
		...
		return [
				...
				{ attributes.childIsEditing == false ?
					<Panel>
						...
						>
							{ attributes.childColumns < 6 ?
								<PanelRow>
									<Button isSecondary onClick={ handleAddColumnBlock }>
										Add Column
									</Button>
								</PanelRow> :
								'' }
							<PanelRow>
								<Button isSecondary onClick={ handleAddRowBlock }>
									Add Row
								</Button>
							</PanelRow>
							<PanelRow>
								<Button isSecondary onClick={ handleRemoveRowBlock }>
									Remove Row
								</Button>
							</PanelRow>
						</PanelBody>
			...
			<div className={ className }>
				...
				{ isSelected && attributes.childIsEditing == false ? // If row block is selected and a child is not selected, show RowMenu
					RowMenu				: // Else, always show RowMenu if the row block is not selected and there are no child columns
