...
const { RichText, InspectorControls, InnerBlocks, BlockControls } = wp.blockEditor;
const { SelectControl, Toolbar, SVG, Path, G, PanelBody, PanelRow } = wp.components;
...
registerBlockType( 'mayflower-blocks/alert', {
	...
	description: __( 'Block for creating text styled as an alert. Should be used sparingly to highlight critical content.' ),
	...
	transforms: {
				...
				attributes: {
					...
					alertText: {
						...
						shortcode: ( attrs, { content } ) => {
							...
							const filtered = content.replace( /(\[alert.*?\]\s*)|(\s*\[\/alert\])/gmi, '' );
		...
		to: [
			...
			},
			{
				...
				transform: ( attributes ) => {
					const alertBlockInnerBlocks = select( 'core/block-editor' ).getSelectedBlock().innerBlocks;
