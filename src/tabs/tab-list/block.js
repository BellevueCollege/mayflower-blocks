...
registerBlockType( 'mayflower-blocks/tab-list', {
	...
	edit: function ({ clientId, className }) {
		...
		const addTab = () => {
			...
			dispatch('core/editor').insertBlock(tabBlock, currentBlockData.innerBlocks.length, clientId);
			...
			dispatch('core/editor').selectBlock(tabBlock.clientId); 
		}
		...
		const updateTab = ( tabClientId ) => {
			dispatch('core/editor').updateBlockAttributes(tabClientId, {tabId: tabClientId });
		}
		...
		const addPanel = ( tabClientId ) => {
			...
			const parentTabsBlock = select('core/editor').getBlock(select('core/editor').getBlockRootClientId( clientId ));
			...
			if (Array.isArray(parentTabsInnerBlocks)) {
				...
				if ( tabContentBlock ) {
					...
					dispatch('core/editor').insertBlock(panelBlock, tabContentBlock.innerBlocks.length, tabContentBlock.clientId);
				}
			}
		}
		...
		return [
			<ul className={`${className} nav nav-tabs`} role="tablist">
				...
				<li role="presentation" class="tab-inserter">
